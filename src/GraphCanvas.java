...
/**
 *  A class to implement a graphical canvas that displays a graph of nodes (represented by points) and edges (represented by lines)
 *
 *  @author  Ha Cao
...
class GraphCanvas extends JComponent {	
	...
	public void paintComponent(Graphics g) {
		...
		for (int i = 0; i < graph.nodes.size(); i++) {
			...
			g.fillOval((int) points.get(i).getX() - 10, (int) points.get(i).getY() - 10, 20, 20);	
		...
		for (int i = 0; i < graph.edges.size(); i++) {
			...
			drawArrowLine(g, ((int) points.get(graph.getEdge(i).getHead().getIndex()).getX()), ((int) points.get(graph.getEdge(i).getHead().getIndex()).getY()), 
					((int) points.get(graph.getEdge(i).getTail().getIndex()).getX()), ((int) points.get(graph.getEdge(i).getTail().getIndex()).getY()));
		}
	}
} // end of GraphCanvas